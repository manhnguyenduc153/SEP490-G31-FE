...
export default function AttendanceReport() {
    ...
    async function loadReport() {
      ...
      try {
        ...
        if (active && res.success && res.data) {
          ...
          setError(res.message ? t(`backendMessages.${res.message}`, { defaultValue: res.message }) : t("attendanceReport.failedToLoadReport"));
          setReportData(null);
        }
      } catch (err) {
        console.error("Failed to load report", err);
        if (active) setError(t("attendanceReport.connectionError"));
      } finally {
  ...
  const getStatusReportBadge = (status: number) => {
    switch (status) {
      case 1:
        return <span className="inline-flex w-5 h-5 items-center justify-center rounded bg-green-100 text-green-700 text-xs font-bold" title={t("attendanceReport.present")}>P</span>;
      case 0:
        return <span className="inline-flex w-5 h-5 items-center justify-center rounded bg-red-100 text-red-700 text-xs font-bold" title={t("attendanceReport.absent")}>A</span>;
      default:
  ...
  const handleExportExcel = () => {
    ...
    const dataToExport = reportData.students.map((st, index) => {
      const rowData: any = {
        "STT": index + 1,
        [t("attendanceReport.studentCode")]: st.studentCode,
        [t("attendanceReport.studentName")]: st.studentName,
        [t("attendanceReport.present")]: st.presentCount,
        [t("attendanceReport.absent")]: st.absentCount,
        [`${t("attendanceReport.attendanceRate")} (%)`]: st.attendanceRate,
      };
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, t("attendanceReport.docTitle"));
  ...
  const handleExportWord = () => {
    ...
    const html = `
      ...
      <h2>${t("attendanceReport.docTitle")}</h2>
      ...
      <div className="p-6 flex-1">
        {!selectedClassId ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-gray-400 py-20 bg-gray-50/50 rounded-xl border border-gray-100">
            <p className="italic">{t("attendanceReport.noData")}</p>
          </div>
        ) : (
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="relative" onMouseLeave={() => setIsExportOpen(false)}>
                ...
                >
                  ...
                  {t("attendanceReport.exportReport")} <ChevronDown className="w-3 h-3 ml-1" />
                </button>
                  ...
                  <div className="absolute top-full right-0 pt-2 z-50">
                    <div className="w-[160px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden">
                       <button onClick={handleExportExcel} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-left text-gray-700 dark:text-gray-200">
                          <Download className="w-4 h-4 text-green-600" /> {t("attendanceReport.excel")}
                       </button>
                       <button onClick={handleExportWord} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-left text-gray-700 dark:text-gray-200">
                          <FileText className="w-4 h-4 text-blue-600" /> {t("attendanceReport.word")}
                       </button>
                       <button onClick={handleExportPdf} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-left text-gray-700 dark:text-gray-200">
                          <FilePdf className="w-4 h-4 text-red-600" /> {t("attendanceReport.pdf")}
                       </button>
                    </div>
                  </div>
                )}
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-gray-205 dark:border-gray-800 text-gray-500 dark:text-gray-400 font-semibold bg-gray-100/80 dark:bg-gray-800/60 sticky top-0 z-10">
                    ...
                    <th className="px-4 py-4 text-center min-w-[100px] text-brand-600 dark:text-brand-400 bg-brand-50/80 dark:bg-brand-900/20">{t("attendanceReport.attendanceRate")}</th>
