                ...
                <div key={item.classId} className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xs dark:border-gray-800 dark:bg-gray-900">
                  ...
                  {isExpanded && (
                    <div className="border-t border-gray-100 p-5 dark:border-gray-800">
                      ...
                      {/* Bảng chi tiết Homework */}
                      {item.homeworks && item.homeworks.length > 0 && (
                        <div className="mt-6 overflow-x-auto">
                          <h5 className="mb-2 text-sm font-bold text-gray-900 dark:text-white">
                            {t("studentScores.homeworkDetails", { defaultValue: "Homework details" })}
                          </h5>
                          <table className="w-full text-sm border border-gray-100 dark:border-gray-800 rounded-lg overflow-hidden">
                            <thead className="bg-gray-50/50 text-xs font-bold uppercase text-gray-500 dark:bg-gray-800/20 dark:text-gray-400">
                              <tr>
                                <th className="px-4 py-2 text-left">{t("studentScores.homeworkTitle", { defaultValue: "Title" })}</th>
                                <th className="px-4 py-2 text-center">{t("studentScores.score", { defaultValue: "Score" })}</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                              {item.homeworks.map((hw) => (
                                <tr key={hw.id}>
                                  <td className="px-4 py-2 text-gray-800 dark:text-gray-100">{hw.title}</td>
                                  <td className="px-4 py-2 text-center">
                                    {hw.score !== null && hw.score !== undefined ? (
                                      <span className="font-medium text-gray-900 dark:text-gray-100">
                                        {Number(hw.score).toFixed(1)} / {Number(hw.totalScore).toFixed(1)}
                                      </span>
                                    ) : (
                      ...
                      {item.exams && item.exams.length > 0 && (
                        <div className="mt-6 overflow-x-auto">
                          <h5 className="mb-2 text-sm font-bold text-gray-900 dark:text-white">
                            {t("studentScores.examDetails", { defaultValue: "Exam details" })}
                          </h5>
                          <table className="w-full text-sm border border-gray-100 dark:border-gray-800 rounded-lg overflow-hidden">
                            <thead className="bg-gray-50/50 text-xs font-bold uppercase text-gray-500 dark:bg-gray-800/20 dark:text-gray-400">
                              <tr>
                                <th className="px-4 py-2 text-left">{t("studentScores.examTitle", { defaultValue: "Title" })}</th>
                                <th className="px-4 py-2 text-center">{t("studentScores.score", { defaultValue: "Score" })}</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                              {item.exams.map((ex) => (
                                <tr key={ex.id}>
                                  <td className="px-4 py-2 text-gray-800 dark:text-gray-100">{ex.title}</td>
                                  <td className="px-4 py-2 text-center">
                                    {ex.score !== null && ex.score !== undefined ? (
                                      <span className="font-medium text-gray-900 dark:text-gray-100">
                                        {Number(ex.score).toFixed(1)} / {Number(ex.totalScore).toFixed(1)}
                                      </span>
                                    ) : (
                                      <span className="text-gray-400 dark:text-gray-500">-</span>
                                    )}
