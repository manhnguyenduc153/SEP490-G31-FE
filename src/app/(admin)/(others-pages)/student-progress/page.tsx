...
export default function StudentProgressPage() {
  ...
  const scoreBarSeries = useMemo(() => [{ name: "Điểm", data: selectedGrade?.components.map((c) => Math.round(c.score * 10) / 10) || [] }], [selectedGrade]);

  const hwDonutOptions: ApexOptions = useMemo(() => ({
    chart: { type: "donut", fontFamily: "Outfit, sans-serif", background: "transparent" },
    colors: ["#10b981", "#f43f5e"], labels: ["Đã nộp", "Chưa nộp"],
    plotOptions: { pie: { donut: { size: "68%", labels: { show: true, total: { show: true, label: "Tổng BT", formatter: () => `${homeworkList.length}`, fontSize: "14px", fontFamily: "Outfit, sans-serif", color: "#374151" } } } } },
    dataLabels: { enabled: false }, legend: { position: "bottom", fontFamily: "Outfit, sans-serif", fontSize: "12px" },
    stroke: { show: false }, tooltip: { y: { formatter: (v: number) => `${v} bài` } },
  }), [homeworkList.length]);

  const attDonutOptions: ApexOptions = useMemo(() => ({
    chart: { type: "donut", fontFamily: "Outfit, sans-serif", background: "transparent" },
    colors: ["#10b981", "#f43f5e"], labels: ["Có mặt", "Vắng mặt"],
    plotOptions: { pie: { donut: { size: "68%", labels: { show: true, total: { show: true, label: "Tổng buổi", formatter: () => `${sessions.length}`, fontSize: "14px", fontFamily: "Outfit, sans-serif", color: "#374151" } } } } },
    dataLabels: { enabled: false }, legend: { position: "bottom", fontFamily: "Outfit, sans-serif", fontSize: "12px" },
    stroke: { show: false }, tooltip: { y: { formatter: (v: number) => `${v} buổi` } },
  }), [sessions.length]);
  ...
  return (
    <div className="space-y-5 w-full pb-10">
      <div className="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-xs dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Tiến độ học tập của tôi</h1>
      ...
      {grades.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-12 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <p className="text-sm text-gray-500">Bạn chưa có lớp học nào có dữ liệu điểm.</p>
        </div>
      ) : (
          ...
          {selectedGrade && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                <StatCard icon={<Award className="w-5 h-5" />} label="Điểm trung bình" value={Number(selectedGrade.averageScore).toFixed(1)} sub="/ 10 điểm"
                ...
                <StatCard icon={<CalendarCheck className="w-5 h-5" />} label="Chuyên cần" value={isLoadingAtt ? "..." : `${attStats.rate}%`} sub={isLoadingAtt ? "" : `${attStats.present}/${sessions.length} buổi có mặt`}
                ...
                <StatCard icon={<CheckCircle2 className="w-5 h-5" />} label="Bài đã nộp" value={isLoadingHw ? "..." : `${hwSubmitted.length}`} sub={isLoadingHw ? "" : `/ ${homeworkList.length} bài tổng cộng`}
                  colorText="text-emerald-600 dark:text-emerald-400" colorBg="bg-emerald-50 dark:bg-emerald-500/10" />
                <StatCard icon={<ClipboardList className="w-5 h-5" />} label="Chưa nộp" value={isLoadingHw ? "..." : `${hwNotSubmitted.length}`} sub={isLoadingHw ? "" : "bài tập còn thiếu"}
              ...
              <div className="rounded-2xl border border-gray-100 bg-white shadow-xs dark:border-white/[0.05] dark:bg-white/[0.03] overflow-hidden">
                <div className="flex items-center gap-1 border-b border-gray-100 dark:border-white/[0.05] bg-gray-50/50 dark:bg-white/[0.02] px-4 py-3">
                  <TabBtn active={activeTab === "scores"}     onClick={() => setActiveTab("scores")}><Award className="w-3.5 h-3.5" /> Điểm số</TabBtn>
                  <TabBtn active={activeTab === "homework"}   onClick={() => setActiveTab("homework")}><ClipboardList className="w-3.5 h-3.5" /> Bài tập</TabBtn>
                  <TabBtn active={activeTab === "attendance"} onClick={() => setActiveTab("attendance")}><CalendarCheck className="w-3.5 h-3.5" /> Chuyên cần</TabBtn>
                </div>
                ...
                {activeTab === "scores" && (
                  <div className="p-5 space-y-5">
                    <p className="text-xs text-gray-400">
                      Lớp: <span className="font-semibold text-gray-600 dark:text-gray-300">{selectedGrade.classCode ? `${selectedGrade.classCode} - ` : ""}{selectedGrade.className}</span>
                    ...
                    <div className="overflow-x-auto rounded-xl border border-gray-100 dark:border-white/[0.05]">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-white/[0.03] text-[11px] font-bold uppercase tracking-wider text-gray-400">
                          <tr><th className="px-4 py-3 text-left">Thành phần</th><th className="px-4 py-3 text-center">Trọng số</th><th className="px-4 py-3 text-center">Điểm số</th></tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-white/[0.04]">
                          ...
                          <tr className="bg-brand-50/40 dark:bg-brand-500/5 font-bold border-t-2 border-brand-100 dark:border-brand-500/20">
                            <td className="px-4 py-3 text-gray-900 dark:text-white">Điểm trung bình</td>
                ...
                {activeTab === "homework" && (
                  <div className="p-5 space-y-5">
                    {isLoadingHw ? (
                      <div className="flex items-center justify-center py-16"><Loader2 className="animate-spin w-5 h-5 text-brand-500 mr-2" /><span className="text-sm text-gray-500">Đang tải bài tập...</span></div>
                    ) : homeworkList.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-14 text-gray-400"><ClipboardList className="w-10 h-10 mb-2 opacity-30" /><p className="text-sm">Lớp này chưa có bài tập nào.</p></div>
                    ) : (
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="flex flex-col gap-3 justify-center">
                            <div className="flex items-center gap-4 rounded-xl border border-emerald-100 bg-emerald-50 dark:bg-emerald-500/10 dark:border-emerald-500/20 p-4">
                              ...
                              <div><p className="text-xs font-semibold text-emerald-700 dark:text-emerald-400">Đã nộp</p><p className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{hwSubmitted.length} <span className="text-sm font-medium text-emerald-400">bài</span></p></div>
                            </div>
                            <div className={`flex items-center gap-4 rounded-xl border p-4 ${hwNotSubmitted.length === 0 ? "border-gray-100 bg-gray-50 dark:bg-white/[0.02] dark:border-white/[0.05]" : "border-rose-100 bg-rose-50 dark:bg-rose-500/10 dark:border-rose-500/20"}`}>
                              ...
                              <div>
                                ...
                                <p className={`text-2xl font-extrabold ${hwNotSubmitted.length === 0 ? "text-gray-400" : "text-rose-600 dark:text-rose-400"}`}>{hwNotSubmitted.length} <span className="text-sm font-medium opacity-70">bài</span></p>
                              </div>
                            </div>
                          </div>
                        ...
                        {hwSubmitted.length > 0 && (
                          <div>
                            <div className="flex items-center gap-2 mb-3"><CheckCircle2 className="w-4 h-4 text-emerald-500" /><p className="text-sm font-bold text-gray-700 dark:text-gray-300">Bài đã nộp ({hwSubmitted.length})</p></div>
                              ...
                              {hwSubmitted.map((hw) => (
                                <div key={hw.id} className="flex items-start justify-between gap-3 rounded-xl border border-emerald-100 bg-emerald-50/50 dark:border-emerald-500/15 dark:bg-emerald-500/5 px-4 py-3">
                                  <div className="min-w-0">
                                    ...
                                    <p className="text-[11px] text-gray-400 mt-0.5">Hạn nộp: {formatDate(hw.dueDate)}{hw.submission?.submitTime && <> &nbsp;·&nbsp; Đã nộp: {formatDate(hw.submission.submitTime)}</>}</p>
                                  </div>
                                  <div className="flex flex-col items-end gap-1 shrink-0">
                                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 px-2.5 py-0.5 text-[10px] font-bold"><CheckCircle2 className="w-3 h-3" /> Đã nộp</span>
                        ...
                        {hwNotSubmitted.length > 0 && (
                          <div>
                            <div className="flex items-center gap-2 mb-3"><XCircle className="w-4 h-4 text-rose-500" /><p className="text-sm font-bold text-gray-700 dark:text-gray-300">Bài chưa nộp ({hwNotSubmitted.length})</p></div>
                            <div className="space-y-2">
                              {hwNotSubmitted.map((hw) => {
                                ...
                                return (
                                  <div key={hw.id} className="flex items-start justify-between gap-3 rounded-xl border border-rose-100 bg-rose-50/50 dark:border-rose-500/15 dark:bg-rose-500/5 px-4 py-3">
                                    <div className="min-w-0">
                                      <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate">{hw.title}</p>
                                      <p className={`text-[11px] mt-0.5 ${overdue ? "text-rose-500 font-semibold" : "text-gray-400"}`}>{overdue ? "⚠ Đã quá hạn: " : "Hạn nộp: "}{formatDate(hw.dueDate)}</p>
                                    </div>
                                    <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400 px-2.5 py-0.5 text-[10px] font-bold shrink-0"><XCircle className="w-3 h-3" /> Chưa nộp</span>
                                  </div>
                                );
                              })}
                            </div>
                ...
                {activeTab === "attendance" && (
                  <div className="p-5 space-y-5">
                    {isLoadingAtt ? (
                      <div className="flex items-center justify-center py-16"><Loader2 className="animate-spin w-5 h-5 text-brand-500 mr-2" /><span className="text-sm text-gray-500">Đang tải dữ liệu chuyên cần...</span></div>
                    ) : attendanceError ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-14 text-gray-400"><CalendarCheck className="w-10 h-10 mb-2 opacity-30" /><p className="text-sm">Chưa có buổi học nào được ghi nhận.</p></div>
                    ) : (
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="flex flex-col gap-3 justify-center">
                        ...
                        <div className="overflow-x-auto rounded-xl border border-gray-100 dark:border-white/[0.05]">
                          <table className="w-full text-sm">
                            <thead className="bg-gray-50 dark:bg-white/[0.03] text-[11px] font-bold uppercase tracking-wider text-gray-400">
                              <tr><th className="px-4 py-3 text-center w-16">Buổi</th><th className="px-4 py-3 text-left">Ngày</th><th className="px-4 py-3 text-center">Trạng thái</th><th className="px-4 py-3 text-left">Ghi chú</th></tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-white/[0.04]">
                              {sessions.map((s) => {
