...
import { attendanceApi, MyAttendanceSessionDto, MyAttendanceClassDto } from "@/services/attendance.api";
import { useTranslation } from "react-i18next";
import {
  Award, RefreshCw, AlertCircle, Loader2, TrendingUp,
...
export default function StudentProgressPage() {
  ...
  // Grades (all classes for student)
  ...
  const [attendanceSessions, setAttendanceSessions] = useState<MyAttendanceSessionDto[]>([]);
  const [attendanceSummary, setAttendanceSummary] = useState<MyAttendanceClassDto | null>(null);
  const [isLoadingAtt, setIsLoadingAtt]     = useState(false);
  const [attendanceError, setAttendanceError] = useState<string | null>(null);

  // ── 1. Load grades on mount ─────────────────────────────────────────────────
  ...
  useEffect(() => {
    async function loadGrades() {
      ...
      try {
        ...
        setGradesError(t("progress.errorSystem"));
      } finally {
  ...
  // ── 2. Load per-class data when class changes ────────────────────────────────
  ...
  const loadClassData = useCallback(async (classId: number) => {
    ...
    try {
      ...
      const withSubs = await Promise.all(
        hws.map(async (hw) => {
          try {
            const subRes = await homeworkApi.getMySubmission(hw.id);
            return { ...hw, submission: subRes.success ? subRes.data : null } as HomeworkWithSub;
          } catch {
    ...
    try {
      const [sessionsRes, summaryRes] = await Promise.all([
        attendanceApi.getMyAttendanceDetails(classId),
      ...
      if (sessionsRes.success && sessionsRes.data) {
        ...
        setAttendanceError(t("progress.errorLoadAttendanceDetail"));
      }

      if (summaryRes.success && summaryRes.data) {
         const clsSummary = summaryRes.data.find(c => c.classId === classId);
         if (clsSummary) {
           setAttendanceSummary(clsSummary);
         }
      }
    } catch {
  ...
  const attStats = useMemo(() => {
    if (attendanceSummary) {
      return {
        ...
        recorded: attendanceSummary.attendedSessions + attendanceSummary.absentSessions,
        present: attendanceSummary.attendedSessions,
        absent: attendanceSummary.absentSessions,
        rate: attendanceSummary.attendanceRate,
      };
    }

    // Fallback calculation from sessions if summary is missing
    const recorded = attendanceSessions.filter((s) => s.status !== -1);
    ...
    return { total: attendanceSessions.length, recorded: recorded.length, present: present.length, absent: absent.length, rate };
  }, [attendanceSessions, attendanceSummary]);
  ...
  if (isLoadingGrades) return (
    <div className="flex flex-col items-center justify-center py-32">
      ...
      <p className="mt-3 text-sm text-gray-500">{t("progress.loadingProgress")}</p>
    </div>
  );
  ...
  if (grades.length === 0) return (
    <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-12 text-center dark:border-gray-700 dark:bg-gray-900">
      ...
      <p className="font-bold text-gray-700 dark:text-gray-300">{t("progress.noClassData")}</p>
      <p className="mt-1 text-sm text-gray-400">{t("progress.noClassesStudent")}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-5 w-full pb-10">
      ...
      <div className="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-xs dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">{t("progress.studentTitle")}</h1>
          <p className="text-xs text-gray-400 mt-0.5">{t("progress.studentSubtitle")}</p>
        </div>
      </div>
                ...
                <div className="p-5 space-y-5">
                  <div>
                    <p className="text-xs text-gray-400 mb-1">
                      Lớp: <span className="font-semibold text-gray-600 dark:text-gray-300">
                                ...
                                <div className="min-w-0">
                                  ...
                                  <p className="text-[11px] text-gray-400 mt-0.5">
                                    ...
                                    {hw.submission?.submitTime && (
                                      <> &nbsp;·&nbsp; Đã nộp: {formatDate(hw.submission.submitTime)}</>
                                    )}
                                  </p>
                                </div>
